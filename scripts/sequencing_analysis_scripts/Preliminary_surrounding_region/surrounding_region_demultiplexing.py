...
#From the raw reads, demultiplex into replicates of the DHFR-PCA screen based on barcodes. The results of this script are available on the SRA
...
#Set this to the folder containing the script. It should also contain the surrounding_region_sample_datasheet_empty.csv file. The common_sequences folder should be in the folder above this one.
os.chdir("/your/path/")
print(os.listdir())
...
universal_seqs_fasta = "../common_sequences/universal_amplicon.fa"
universal_seqs = get_dict_of_seq(universal_seqs_fasta)
print(universal_seqs)

indexes_fasta = "../common_sequences/indexes.fa"
...
#%%Convert plate arrays to Dataframe for data analysis
plate_1_map = './plate_1_DMS.csv'



plate_1 = pd.read_csv(plate_1_map, index_col=0)
...
for column in plate_1.columns:
    for row in plate_1.index:
        
        pcr_dict = {}
        
        pcr = plate_1.loc[row][column]
        
        #if pcr not in [np.nan]:
        if pcr == "Pos" or pcr == "GP3" or pcr == "GP4" :
            
           
                
            pcr_dict['Pool'] = pcr
            pcr_dict['RC_for_index'] = int(row)
            pcr_dict['RC_rev_index'] = int(column)
            pcr_dict['PE1_index'] = 2
            pcr_dict['PE2_index'] = 3


            dict_of_dict[sample_id] = pcr_dict

            sample_id += 1
        

#%%Make the first sample dataframe, the barcodes for each pool only
sample_dataframe = pd.read_csv("./surrounding_region_sample_datasheet_empty.csv", index_col=0)
...
sample_dataframe.to_csv('./surrounding_region_sample_datasheet_demultiplexed_merged.csv', sep=',')